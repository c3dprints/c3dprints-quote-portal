...
def build_customer_quote_text(quantity, hours, grams, suggested_sell_per_unit, total_sell, include_shipping):
    ...
    return (
        f"Hi! Thanks for sending over the details.\n\n"
        f"{price_line}\n\n"
...
def calculate_quote(
    ...
    nozzle_wear = (settings.nozzle_cost / settings.nozzle_hours) * hours if settings.nozzle_hours > 0 else 0
    sheet_wear = settings.sheet_cost / settings.sheet_prints if settings.sheet_prints > 0 else 0
    ...
    return {
        ...
        "totals": {
            ...
            "cost_to_make": money(adjusted_cost_per_unit * quantity),
            "tax": money(tax_per_unit * quantity),
            "suggested_sell_price": money(suggested_sell_per_unit * quantity),
            "profit": money(profit_per_unit * quantity),
        },
        "breakdown_per_unit": {
        ...
        "customer_quote": build_customer_quote_text(
            quantity, hours, grams, suggested_sell_per_unit, suggested_sell_per_unit * quantity, include_shipping
        ),
    }
